...
public class AttackResolver {
	...
	public void calculateDamages(SkillArea skillArea, Integer targetSize) {
		...
		Integer targetIndex = skillArea.getAttacks().get(0).getTarget();
		...
		skillArea.setDamages(damages);
	}
	
	public void resolveAttack(GameState gameState, PlayersRequest playersRequest) {
		String defender = playersRequest.getPlayer1();
		Integer monsterIndex = -1;
		for (Monster monster : gameState.getPlayerArea(defender).getMonsters()) {
			Integer damage = gameState.getSkillArea().getDamages().get(monsterIndex);
	 ...
	 * @param gameState
	 */
//	public void resolveAttackOld(GameState gameState) {
//		// get info from the game state
//		SkillRequest attackRequest = gameState.getAttackRequest();
//		DefenseRequest defendRequest = gameState.getDefendRequest();
